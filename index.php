...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN"
			...
			function go(url) { /* It gets executed as soon as the user clicks "Shorten" or he hits enter */
				$.post('url.php', { url_php/* url in php file */: url/* url in this file */ }, function(data){ /* post the keyed in text to url.php */
					if (data == 'error_no_url'){/* if the data returned by php file equals this output */
						$('#message').html('<p><strong>No URL specified</strong></p>').css("color","red"); /* show this message to the user, on the <div id="message"> tag */
					}
					else if (data == 'error_invalid_url'){/* if the data returned by php file equals this output */
						$('#message').html('<p><strong>Not a valid URL</strong></p>').css("color","red");
					}
						...
						$('#message').html('<p><strong>Successfully shortened your URL</strong></p>').css("color","green"); /* show this message to the user, on the <div id="message"> tag*/
					}
				});
			}
		</script>
	...
	<body>
		<div id="container">
		  ...
		  <div id="copyright"><p>Karadimas Andreas @ <?php echo date('Y'); ?></p></div>
		  </div>
	</body>
</html>
